...
namespace Course.Service.Services.Implementations
{
    public class StudentService : IStudentService
    {
        ...
        public StudentService(IGroupRepository groupRepository,IStudentRepository repository, IWebHostEnvironment environment, ILogger<GroupService> logger)
        {
        ...
        public int Create(StudentCreateDto dto)
        {
            ...
            var student = Mapper<StudentCreateDto, Student>.Map(dto);
            student.Photo = file;
        ...
        public List<StudentGetDto> GetAll(string? search = null)
        {
            ...
            var students = _studentRepository.GetAll(x => x.FullName.Contains(search), "Group").ToList();

            var studentDtos = students.Select(student =>
            {
                var dto = Mapper<Student, StudentGetDto>.Map(student);
                dto.GroupName = student.Group.No;
                dto.PhotoUrl = student.Photo;
                return dto;
            }).ToList();
        ...
        public StudentGetDto GetById(int id)
        {
            ...
            StudentGetDto dto = Mapper<Student, StudentGetDto>.Map(student);
            dto.GroupName = student.Group.No;
            dto.PhotoUrl = student.Photo;
